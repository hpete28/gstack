...
name: ship
description: |
  Complete authorized release preparation, repair scoped failures, run gates, commit, push and create a PR.
---
...
- In-branch test failures that remain unresolved after scoped diagnosis and repair; never push a failing revision
...
**Never stop for:**
- Uncommitted changes within the authorized task (preserve unrelated work and stage only owned files)
...
7. **After all fixes (auto + user-approved):**
   - If ANY fixes were applied: commit fixed files by name (`git add <fixed-files> && git commit -m "fix: pre-landing review fixes"`), then rerun affected tests and review the repaired diff in this same authorized shipping operation. Continue through the remaining gates and requested release steps when they pass. Do not require another `/ship` invocation.
